...
interface IItemProps {
  ...
  // >> 전체 장ㅂ바구니 순회 + 매개변수의 id와 일치하지 않는 제품만 새로운 배열에 저장
...
function Practice() { 
  ...
  const handleCreateItem = (name: string, amount: number) => {
    ...
    nextId.current++; //item(상태 변수)의 변경 여부와 상관없이 값이 유지
  }
  ...
  return (
    <div style={{
      ...
      {initialItems.map((item) => (
        ...
        <Item item={item} key={item.id} onRemove={handleRemoveItem} onUpdate={handleUpdateAmount}/>
      ))}
    </div>
  )
}
