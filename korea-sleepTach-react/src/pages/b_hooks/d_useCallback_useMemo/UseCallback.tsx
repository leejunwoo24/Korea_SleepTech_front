...
// ! === useCallback === (함수의 주소값을 변경시키지 않는 기능)
// : React 함수형 컴포넌트 Hook 
// : 특정 콜백함수가 의존성 배열에 명시된 값들이 변경되지 않는 한
// , 동일한 함수 인스턴스를 유지하도록 해주는 훅
// >> 콜백 함수를 메모제이션(Memoization)
// >> 주로) 자식 컴포넌트에 함수 전달 시 사용

//? React 컴포넌트 리랜더링 
// : 함수는 컴포넌트가 리렌더링(상태 변화 || props값 변경 등)될 때마다 새로운 함수 인스턴스가 생성
...
const Button = ({handleClick}: {handleClick: () => void}) => {
  ...
  return <button onClick={handleClick}>자식버튼</button>
}
