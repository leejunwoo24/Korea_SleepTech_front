...
? React 컴포넌트의 '라이프 사이클'
  ...
  2) 업데이트: 컴포넌트 재렌더링 (주로, state, props의 변경에 의해 발생)
...
useEffect(() => {
  ...
  ? useEffect의 정리 함수
  : useEffect에서 함수를 return하면 해당 함수는 컴포넌ㅌ츠가 화면에서 사라질 때 (언마운트) 실행
...
function UseEffect01() {
  ...
  return (
    <div>
      ...
      */}
      <button onClick={() => setName(name === '이승아' ? "이도경" : "이승아")}>이름변경</button>
