...
function practice01() {
  ...
  useEffect(() => {
    async function setupWebcam() { // 비동기 함수로 웹캠 설정
      ...
      if(videoRef.current){
        // 비디어 ref가 현재 참조하는 DOM 요소가 있다면
        videoRef.current.srcObject = stream; // 해당 비다오 요소의 소스로 스트림을 설정
      }
    }
    setupWebcam(); // 위에서 정의한 웹갬 설정 함수 호출
  }, []); // 빈 의존성 배열: 컴포넌트 마운티 시에만 효과 실행
