  ...
  return (
    <div>
      ...
      <div css={s.container}>
        ...
        <input type="text" placeholder='Add a new task' onKeyDown={handleKeyPrass} css={s.input}/>
        <div>
          <button css={s.button} onClick={() => setFilter('all')}>All(모든)</button>
          <button css={s.button} onClick={() => setFilter('active')}>Active(완료 전)</button>
          <button css={s.button} onClick={() => setFilter('completed')}>Completed(완료 후)</button>
        </div>
        <ul css={s.todoList}>
          {filteredTodos.map(todo => (
            <li key={todo.id} css={s.todoItem}>
              ...
              <button onClick={() => handleDeleteTodo(todo.id)} css={s.deleteButton}>삭제</button>
            </li>
          ))}
        </ul>
      </div>
