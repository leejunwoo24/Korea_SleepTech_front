...
function addTodo(todos: ITodoItem[], task: string): ITodoItem[] {
  const newTodo: ITodoItem = {
    ...
    // - 인자 내에 배열이 포함된 경우 배열 요소 중에서 가장 큰 값을 반환

    // Math.max(0, ...todos.map(todo => todo.id))
    // : 현재 할 일 중 가장 큰 id값이 반환

    // cf) ...todos.map(todo => todo.id)
    // : 현재 할 일 목록을 순회하여 id값만 추출

    // 1번째 인자 0은 리스트가 비워진 경우의 기본값
    // : 가장 작은 id값은 1
  ...
  //? 배열의 불변성
  // : 기존의 배열에 요소를 추가하는 경우(push) 배열의 변환를 '리액트'에서 인지하지 못 함
...
function deleteTodo(todos: ITodoItem[], id: number): ITodoItem[]{
  const chageTodos = todos.filter(todo => todo.id !== id);
