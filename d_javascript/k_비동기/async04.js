...
// ! 비동기 작업과 예외 처리
// : 외부와 연결되는 작업은 주로 '비동기 작업'
// : 예외 처리 코드와 함께 사용 '권장'
async function fetchCustomerData() {
  try{
    // 실제 로직이 구현되는 목록

    let data = await fetch('url') // fetch() 함수: Response 객체

    if(!data.ok){
      throw new Error('네트워크 응답이 실패되어 데이터를 정상적으로 가져오지 못 하였습니다.')
    }
...
// & Response 객체
// 1) ok
// : HTTP 상태 코드가 200~299 범위에 있을 때 true를 반환
// >> 요청이 성공적으로 완료 되었음을 반환

// 2) status
// : 응답의 HTTP 상태코드를 반환 (예 200, 404, 500등)
