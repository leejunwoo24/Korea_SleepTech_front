...
function addTodo(content){ // 함수 호출 시 인자로 할 일의 내용 전달받음
  // 새로운 할일 생성
  const newTodo = {
    id: nextId, // 이전 배열 길이 + 1 === 마지막 인덱스 번호 + 1
    content: content,
    completed: false
  };

  nextId++; // 다음 할 일 추가시 사용할 고유 ID값 증가
  todos.push(newTodo);
...
function toggleTodo(id){
  ...
  todos = todos.map(todo => {
    ...
    if (todo.id === id) {
      ...
      // cf) 기존 속성키: 새로운 속성값; >> 객체 내부에서 속성값 재할당
      return{...todo, completed: !todo.completed}; // {...todo} >> todo의 요소만 가져와서 새로운 객체 생성
    }

    return todo; // id가 일치하지 않는데이터는 기존 객체 그대로 반환 
  });
